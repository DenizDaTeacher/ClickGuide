-- First, check existing constraints and modify step_ids to be unique per project
-- Delete any existing steps for our projects
DELETE FROM public.call_steps 
WHERE tenant_id IN ('DEMO', 'MEDIAMARKTSATURN', 'EON', 'DHL', '1&1') 
AND workflow_name = 'Gesprächsschritte';

-- Insert unique steps for each project using project-specific step_ids
...
('one1_greeting', 'Begrüßung', 'Freundliche Begrüßung und Firmenvorstellung', 'Guten Tag! Hier ist [Name] von [Firma]. Wie kann ich Ihnen heute helfen?', true, 1, '1&1', 'Gesprächsschritte', 'Begrüßung', 'normal', '[{"id": "action-1", "label": "Weiter zum nächsten Schritt", "variant": "default", "actionType": "complete", "icon": "✓", "enabled": true}]'),
('one1_identification', 'Kundenidentifikation', 'Sicherheitsabfrage zur Identitätsprüfung', 'Zur Sicherheit benötige ich von Ihnen bitte Ihren vollständigen Namen und Ihr Geburtsdatum.', true, 2, '1&1', 'Gesprächsschritte', 'Authentifizierung', 'normal', '[{"id": "action-2", "label": "Kunde identifiziert", "variant": "default", "actionType": "complete", "icon": "✓", "enabled": true}]'),
('one1_verification', 'Datenverifikation', 'Abgleich mit den im System hinterlegten Daten', 'Ich prüfe nun Ihre Angaben in unserem System...', true, 3, '1&1', 'Gesprächsschritte', 'Verifikation', 'normal', '[{"id": "action-3", "label": "Daten verifiziert", "variant": "default", "actionType": "complete", "icon": "✓", "enabled": true}]');